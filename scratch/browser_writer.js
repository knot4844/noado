...
export async function generateArticleWithBrowser(targetKeyword, targetCategory, feedback = null) {
  ...
  try {
    ...
    // Random simulators to force Gemini to create distinct content layouts
    const names = ["김영수", "박순자", "이갑득", "최정희", "정태호", "강복순", "윤상철", "임옥분"];
    const ages = [66, 68, 71, 73, 65, 69, 72, 75];
    const cities = ["대구 수성구", "부산 영도구", "인천 부평구", "광주 북구", "대전 유성구", "울산 남구", "강원 강릉시", "충북 청주시"];
    const assets = [
      "공시지가 2억 4천만 원의 아파트를 한 채 보유하고 있으며 국민연금으로 매달 42만 원을 수령",
      "시가 3억 원 상당의 주택과 은행 예적금 3,500만 원을 보유 중이며 연금 외 다른 근로 소득은 없는",
      "소득은 전혀 없으나 자녀 명의의 주택(공시가격 6억 원 이상)에 거주하고 있으며 개인 연금으로 월 15만 원을 수령하는",
      "보증금 5,000만 원에 월세 30만 원 빌라에 거주하며 은행 이자 소득이 연간 150만 원 발생하고 있는"
    ];
    const layouts = [
      "마크다운 표(Table)를 글의 중간(## 2 단락 아래)에 배치하고 FAQ를 4개로 상세하게 작성하십시오.",
      "신청 자격 기준을 번호 매기기가 아닌 큼직한 이모티콘 체크 리스트로 먼저 요약해 보여준 다음 마크다운 표(Table)를 ## 3 단락 하단에 배치하십시오.",
      "방문 신청 필요 서류 목록을 가장 상단에 구체적으로 강조하고 모바일/온라인 절차를 단계별(1단계, 2단계 등)로 상세히 구성해 주십시오."
    ];

    const rName = names[Math.floor(Math.random() * names.length)];
    const rAge = ages[Math.floor(Math.random() * ages.length)];
    const rCity = cities[Math.floor(Math.random() * cities.length)];
    const rAsset = assets[Math.floor(Math.random() * assets.length)];
    const rLayout = layouts[Math.floor(Math.random() * layouts.length)];
...
7. [매우 중요 - 시뮬레이션 사례 필수] 본문 소제목 중 하나는 반드시 가상의 은퇴자 사례인 **"${rCity}에 거주하는 만 ${rAge}세 ${rName} 어르신(${rAsset})"**의 구체적 케이스를 설정하여 실제 소득인정액 모의 계산 방식 및 예상 혜택 수령액을 자세히 계산해주는 모의 시뮬레이션 가이드라인 단락으로 채우십시오.
...
9. 글 구성 다양화 규칙: 이번 글은 특별히 **"${rLayout}"** 규칙을 준수하여 작성해 주십시오.
