...
// URL/Slug friendly string generator with NFC normalization & length limits
function getSlug(text) {
  // Always NFC normalize Korean characters
  let slug = text
    .normalize('NFC')
    .toLowerCase()
  ...
  // Shorten slug mapping to prevent ENAMETOOLONG failures in Linux/Vercel
  ...
  if (filenameAbbrMap[slug]) {
    return filenameAbbrMap[slug];
  }
  if (slug.length > 25) {
...
function evaluateSEO(articleText, targetKeyword) {
  ...
  if (charCount >= 2300) {
    score += 20;
    reports.push(`✅ 본문 글자 수(공백 제외)가 ${charCount}자로 충분합니다. (기준 2300자 이상) (+20)`);
  } else {
    reports.push(`❌ 본문 글자 수가 ${charCount}자로 부족합니다. (최소 2300자 필요, 현재: ${charCount}자)`);
  }
  ...
  if (h2Count >= 4) {
    ...
    reports.push(`❌ H2 소제목이 ${h2Count}개로 부족합니다. (최소 4개 이상 권장, 현재: ${h2Count}개)`);
  }
  ...
  if (adCount >= 2) {
    score += 15;
    reports.push(`✅ 애드센스 광고 컨테이너가 ${adCount}개로 적절하게 삽입되었습니다. (+15)`);
  } else {
    reports.push(`❌ 본문 내 애드센스 광고 영역(adsense-container)이 부족합니다. (본문 중간 2개 이상 배치 필요, 현재: ${adCount}개)`);
  }
