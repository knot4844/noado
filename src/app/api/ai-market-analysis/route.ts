...
[요청 사항]
요청된 소재지를 기반으로 주변 (반경 1km 내) 상권, 평균 이용료(원룸/투룸/단기임대 등), 공실률 동향, 최근 3개월의 부동산 시장 트렌드를 가상으로 분석하여 프로페셔널한 "주변 시세 컨설팅 리포트" 문서 형태로 답변해 주세요.

[출력 양식 준수 (Markdown)]
1. **📍 지역 시세 브리핑** (평균 보증금/월 이용료 요약)
2. **📈 수요 및 공실률 동향** (최근 유동인구 및 임차 문의 동향)
3. **💡 적정 이용료 설정 가이드 및 AI 제안** (현재 상황에서 수익을 극대화할 수 있는 월 이용료/관리비 세팅 조언)
