  ...
  if (contract.tenant_phone) {
    try {
      alimtalkSent = await sendKakaoAlimtalk({
        ...
        to: normalizePhone(contract.tenant_phone),
        variables: {
      ...
      console.error('[contracts/send] 알림톡 발송 에러:', e)
    }
    ...
    await supabaseAdmin.from('notification_logs').insert({
      owner_id:        user.id,
      room_id:         null,
      template_key:    'CONTRACT_SIGN',
      recipient_name:  contract.tenant_name  ?? null,
      recipient_phone: normalizePhone(contract.tenant_phone),
      status:          alimtalkSent ? 'success' : 'failed',
    })
  }

  console.log(`[contracts/send] 발송 완료: contractId=${contractId}, alimtalk=${alimtalkSent}`)

  return NextResponse.json({
    ok: true,
    signLink,
    alimtalkSent,
    message: alimtalkSent
      ? '계약서가 발송되고 알림톡이 전송되었습니다.'
      : '계약서가 발송되었습니다. (알림톡 미발송 — 연락처 확인 필요)',
  })
}
